...
declare
  ...
  s_row        md.schedule%rowtype;
  wds_row      md.week_day_schedule%rowtype;
...
begin
  ...
  ---https://jira.mos.social/browse/MDP-1724
  ...
  returning id into revision_cnt;
  update md.schedule
  set start_time=lid_row.start_time,
      end_date=lid_row.end_time,
      place_id=lid_row.place_id
  where id = lid_row.schedule_id
  returning * into s_row;
  upd_id := s_row.id;
  update audit.schedule_aud
  set revend=revision_cnt,
      revend_timestamp=current_timestamp
  where id = upd_id
    and revend isnull;
  insert into audit.schedule_aud(id, rev, revtype, end_date, end_time, pause, start_date, start_time, group_id,
                                 place_id)
  values (s_row.id, revision_cnt, 1, s_row.end_date, s_row.end_time, s_row.pause, s_row.start_date,
          s_row.start_time, s_row.group_id, s_row.place_id);
  ----week_day_schedule
  update md.week_day_schedule
  set start_time=lid_row.start_time,
      end_date=lid_row.end_time
  where schedule_id = s_row.id
  returning * into wds_row;
  revision_cnt := nextval('public.hibernate_sequence');
  insert into audit.revision (id, timestamp, user_id)
  values (revision_cnt, (trunc(extract(epoch from now()) * 1000)), 1)
  returning id into revision_cnt;
  ...
  upd_id := wds_row.id;
  update audit.week_day_schedule_aud
  set revend=revision_cnt,
      revend_timestamp=current_timestamp
  where id = upd_id
    and revend isnull;
  
  insert into audit.week_day_schedule_aud(id, rev, revtype, day_of_week, end_date, end_time, pause, start_date,
                                          start_time, place_id, schedule_id)
  values (wds_row.id, revision_cnt, 1, wds_row.day_of_week, wds_row.end_date, wds_row.end_time, wds_row.pause,
          wds_row.start_date, wds_row.start_time,
          wds_row.place_id, wds_row.schedule_id);
  ----расписание преподователей
