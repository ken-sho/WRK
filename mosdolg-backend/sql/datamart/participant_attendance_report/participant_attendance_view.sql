  ...
  return query
    ...
    where (case
      ...
      and ((case
              ...
              else i_p_tcso_s_ch_ids_1 in (select distinct to3.territory_id
...
$$;create function participant_attendance_view(p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date, p_area_ids bigint[] DEFAULT ARRAY[]::integer[], p_district_ids bigint[] DEFAULT ARRAY[]::integer[], p_tcso_s_ch_ids bigint[] DEFAULT ARRAY[]::integer[], p_tcso_coordinators_ids bigint[] DEFAULT ARRAY[]::integer[], p_tcso_creator_ids bigint[] DEFAULT ARRAY[]::integer[], p_uszn_ids bigint[] DEFAULT ARRAY[]::integer[], p_provider_ids bigint[] DEFAULT ARRAY[]::integer[], p_limit_value integer DEFAULT NULL::integer, p_offset_value bigint DEFAULT NULL::bigint)
  returns TABLE(id bigint, fio_participant text, tel text, participant_last_status character varying, cr_last_status character varying, date_enrolled date, cso_short_title text, tcso_district text, tcso_area text, supplier_short_title text, supplier_dept text, contract_number text, activity_title_l1 text, activity_title_l2 text, activity_title_l3 text, group_code text, tc_short_title text, group_status text, fio_coordinator text, list_teachers_fio text, cnt_lesson bigint, cnt_attended bigint, cnt_with_skm bigint, cnt_without_skm bigint, cnt_web_coordinator bigint, cnt_system_auto_confirmation bigint, cnt_missed bigint, percent_attendance numeric, status_attendance text)
  language plpgsql
as $$
declare
  i_p_date_from date;
