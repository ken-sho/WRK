...
config_file|/var/lib/pgsql/11/data/postgresql.conf|Sets the server's main configuration file.
...
data_directory|/var/lib/pgsql/11/data|Sets the server's data directory.
...
hba_file|/var/lib/pgsql/11/data/pg_hba.conf|Sets the server's "hba" configuration file.
...
ident_file|/var/lib/pgsql/11/data/pg_ident.conf|Sets the server's "ident" configuration file.
...
maintenance_work_mem|768MB|Sets the maximum memory to be used for maintenance operations.
...
search_path|public|Sets the schema search order for names that are not schema-qualified.
...
server_version|11.3|Shows the server version.
server_version_num|110003|Shows the server version as an integer.
...
shared_buffers|3GB|Sets the number of shared memory buffers used by the server.
...
work_mem|7864kB|Sets the maximum memory to be used for query workspaces.
