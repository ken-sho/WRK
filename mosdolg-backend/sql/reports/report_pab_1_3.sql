drop function if exists public.report_pab_1_3;
CREATE OR REPLACE FUNCTION public.report_pab_1_3()
  returns TABLE (
    ...
    groups_continued_start                numeric,
    groups_continued_paused               numeric,
    groups_continued_restart              numeric,
    groups_continued_waiting              numeric,
    groups_continued_waiting_full         numeric,
    groups_continued_waiting_0_50         numeric,
    groups_continued_waiting_51_80        numeric,
    groups_continued_waiting_81_99        numeric,
    groups_continued_created              numeric,
    groups_continued_null                 numeric,
    groups_continued_with_participant     numeric,
    groups_without_people                 numeric,
    groups_without_people_month           numeric,
    groups_without_people_started         numeric,
    groups_without_people_waiting         numeric,
    groups_without_people_not_started     numeric,
    groups_on_pause                       numeric,
    groups_finished                       numeric,
    groups_overbooked                     numeric,
    group_places_total                    numeric,
    group_places_vacant                   numeric,
    group_places_occupied_without_paused  numeric,
    group_places_occupied                 numeric,
    group_places_occupied_paused          numeric,
    group_places_closed                   numeric,
...
begin
  ...
  return query
    with
      ...
      R7 as (
        select count(L2_0.id)::numeric as groups_continued
        from L2_0
      ),
      R7_1 as (
        select count(L2_0.id)::numeric as groups_continued_start
        from L2_0
        where L2_0.last_status = 8
      ),
      R7_2 as (
        select count(L2_0.id)::numeric as groups_continued_paused
        from L2_0
        where L2_0.last_status = 9
      ),
      R7_3 as (
        select count(L2_0.id)::numeric as groups_continued_restart
        from L2_0
        where L2_0.last_status = 11
      ),
      R7_4 as (
        select count(L2_0.id)::numeric as groups_continued_waiting
        from L2_0
      ...
      R7_5 as (
        select count(L2_1.id)::numeric as groups_continued_waiting_full
        from L2_1
        where L2_1.cnt_part >= L2_1.max_count
      ),
      R7_6 as (
        select count(L2_1.id)::numeric as groups_continued_waiting_0_50
        from L2_1
        where L2_1.cnt_part <= L2_1.max_count * 0.5
      ),
      R7_7 as (
        select count(L2_1.id)::numeric as groups_continued_waiting_51_80
        from L2_1
        where L2_1.cnt_part > L2_1.max_count * 0.5
          and L2_1.cnt_part <= L2_1.max_count * 0.8
      ),
      R7_8 as (
        select count(L2_1.id)::numeric as groups_continued_waiting_81_99
        from L2_1
        where L2_1.cnt_part > L2_1.max_count * 0.8
          and L2_1.cnt_part < L2_1.max_count
      ),
      R7_9 as (
        select count(L2_0.id)::numeric as groups_continued_created
        from L2_0
      ...
      L2_2 as (
        select g.id,
               gsr.last_status,
               count(crsr.class_record_id)                                    as cnt_patr_ingroup,
      ...
      R7_10 as (
        select count(distinct L2_2.id)::numeric as groups_continued_null
        from L2_2
        where cnt_patr_ingroup - cnt_patr_null = 0
      ),
      R7_11 as (
        select count(distinct L2_2.id)::numeric as groups_continued_with_participant
        from L2_2
      ...
      R8 as ( -- группы без активных участников
        select count(distinct R8_all_stt.group_id)::numeric as groups_without_people
        from R8_all_stt
        where last_status in (3, 4, 6, 7, 8, 11, 12)
      ),
      R8_1 as (
        select count(distinct R8_all_stt.group_id)::numeric as groups_without_people_month
        from R8_all_stt
        where last_status in (3, 4, 6, 8, 11)
          and cr_isnull = true
      ),
      R8_2 as (
        select count(distinct R8_all_stt.group_id)::numeric as groups_without_people_started
        from R8_all_stt
        where last_status in (8, 11, 12)
      ),
      R8_3 as (
        select count(distinct R8_all_stt.group_id)::numeric as groups_without_people_waiting
        from R8_all_stt
        where last_status = 7
      ),
      R8_4 as (
        select count(distinct R8_all_stt.group_id)::numeric as groups_without_people_not_started
        from R8_all_stt
      ...
      R11 as (
        select count(*)::numeric as groups_overbooked
      ...
      R12 as (
        ...
        from (
               select g.id,
                      g.max_count -
                      sum(case when crsr.last_status isnull then 0 else 1 end)::numeric as group_places_vacant
               from L0 g
                      join REG_groups gsr on g.id = gsr.group_id
                      left join md.class_record cr on g.id = cr.group_id
                      left join REG_cr crsr on cr.id = crsr.class_record_id and crsr.last_status in (1, 3, 6, 7)
               where (gsr.last_status in (8, 9, 11, 12) and g.extend = true)
                  or (gsr.last_status in (3, 4, 6, 7))
               group by g.id, g.max_count
             ) t
      ...
      R13 as (
        select count(1)::numeric as group_places_occupied_without_paused
      ...
      R13_1 as (
        select count(1)::numeric as group_places_occupied_paused
        from md.class_record cr
               join REG_cr crsr on cr.id = crsr.class_record_id
        where crsr.last_status = 2
      ),
      R13_2 as (
        select sum(case when t.group_places_closed < 0 then 0 else t.group_places_closed end) as group_places_closed
        from (
               select g.id,
                      g.max_count -
                      sum(1) as group_places_closed
               from L0 g
                      join REG_groups gsr on g.id = gsr.group_id
                      join md.class_record cr on g.id = cr.group_id
                      join REG_cr crsr on cr.id = crsr.class_record_id
               where gsr.last_status in (8, 9, 11, 12)
                 and g.extend = false
      ...
      -- @formatter:off
select * from R1, R2, R3, R4, R5, R6, R7, R7_1, R7_2, R7_3, R7_4,
              R7_5, R7_6, R7_7, R7_8,R7_9, R7_10, R7_11,
              R8, R8_1, R8_2, R8_3, R8_4, R9, R10, R11, R11_1,-- groups
              R12, R13, R13_0, R13_1, R13_2,  -- group places
