create function report_pab_r10(district_1 bigint[] DEFAULT ARRAY[]::integer[], district_2 bigint[] DEFAULT ARRAY[]::integer[], district_3 bigint[] DEFAULT ARRAY[]::integer[], district_4 bigint[] DEFAULT ARRAY[]::integer[], tcso_1 bigint[] DEFAULT ARRAY[]::integer[], tcso_2 bigint[] DEFAULT ARRAY[]::integer[], tcso_3 bigint[] DEFAULT ARRAY[]::integer[], stt_1 bigint[] DEFAULT ARRAY[]::integer[], stt_2 bigint[] DEFAULT ARRAY[]::integer[], stt_3 bigint[] DEFAULT ARRAY[]::integer[], sign_online text DEFAULT ''::text, arr_act_1 bigint[] DEFAULT ARRAY[]::integer[], arr_act_2 bigint[] DEFAULT ARRAY[]::integer[], arr_act_3 bigint[] DEFAULT ARRAY[]::integer[])
  returns TABLE(rownum bigint, fio text, participantid bigint, dateofbirth text, gender text, age double precision, livingregion text, registrationregion text, creatortcso text, creationdate text, participantstatus text, rejectionreason text, firstlevelactivity text, secondlevelactivity text, thirdlevelactivity text, activityprofilecreationdate text, activitycreatortcso text, groupid text, groupstatus text, classrecordstatus text, enrollmentdate text, stopreason text, leavingdate text, leavingreason text, isonline text, providershortname text, tcsoprovidershortname text, tcsoproviderregion text, tcsoproviderdistrict text)
  language plpgsql
as $$
begin
  ...
  return query
    with
      ...
      reg_cr as (
        ...
        where crsr.start_date is not null
        group by cr.id
      )
    select --rownum
    ...
    where (case
      ...
      and (case
             when arr_act_1[1] isnull then true
               ...
               ACT.act3_num in (select * from unnest(arr_act_1)) end)
      and (case
      ...
      and (case
             when arr_act_3[1] isnull then true
               ...
               ACT.act1_num in (select * from unnest(arr_act_3)) end)
      
      and coalesce(ACT.activity_type, '') =
          (case when upper(sign_online) = '' then coalesce(ACT.activity_type, '') else upper(sign_online) end)
