drop function if exists public.report_pab_1_2;
CREATE OR REPLACE FUNCTION public.report_pab_1_2()
  returns TABLE (
  ...
  return query
    with
      ...
      R7_1 as (
        ...
        from L2_0
        where L2_0.last_status in (8, 12)
      ),
      R7_2 as (
      ...
      L2_1 as (
        select g.id, gsr.last_status, crsr.last_status as last_status_part
        from L0 g
               join REG_groups gsr on g.id = gsr.group_id
               left join md.class_record cr on g.id = cr.group_id
               left join REG_cr crsr on cr.id = crsr.class_record_id and crsr.last_status not in (4, 5)
        where (gsr.last_status in (8, 9, 11, 12) and g.extend = true)
           or (gsr.last_status in (3, 4, 6, 7)) --and g.extend = false)
      ),
      R7_4 as (
        select count(distinct L2_1.id)::numeric as groups_continued_null
        from L2_1
        where L2_1.last_status in (3, 4, 6, 7)
          and L2_1.last_status_part is null
      ),
      R7_5 as (
        select count(distinct L2_1.id)::numeric as groups_continued_with_participant
        from L2_1
        where L2_1.last_status in (3, 4, 6, 7)
          and L2_1.last_status_part is not null
      ),
      ...
      R12 as (
        ...
        from (
               ...
               where gsr.last_status in (3, 4, 6, 7, 8, 9, 11, 12)
                 and crsr.last_status in (1, 3, 6, 7) --(1, 2, 3, 6, 7)
      ...
      -- @formatter:off
select * from R1, R2, R3, R4, R5, R6, R7, R7_1, R7_2, R7_3, R7_4, R7_5,
              R8, R9, R10, -- groups
              R11, R12, R13_0, R13, R13_1,  -- group places
