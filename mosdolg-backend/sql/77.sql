...
application_name|DataGrip 2022.1.4|Sets the application name to be reported in statistics and logs.
...
config_file|/var/lib/pgsql/10/data/postgresql.conf|Sets the server's main configuration file.
...
effective_cache_size|4608MB|Sets the planner's assumption about the total size of the data caches.
...
hba_file|/var/lib/pgsql/10/data/pg_hba.conf|Sets the server's "hba" configuration file.
...
ident_file|/var/lib/pgsql/10/data/pg_ident.conf|Sets the server's "ident" configuration file.
...
log_filename|postgresql-%H%M.log|Sets the file name pattern for log files.
log_hostname|off|Logs the host name in the connection logs.
log_line_prefix|%t [%p]: user=%u,db=%d,app=%a,client=%h |Controls information prefixed to each log line.
...
log_rotation_age|12h|Automatic log file rotation will occur after N minutes.
log_rotation_size|100MB|Automatic log file rotation will occur after N kilobytes.
log_statement|mod|Sets the type of statements logged.
...
log_timezone|Europe/Moscow|Sets the time zone to use in log messages.
...
maintenance_work_mem|384MB|Sets the maximum memory to be used for maintenance operations.
...
server_version|10.9|Shows the server version.
...
shared_buffers|1536MB|Sets the number of shared memory buffers used by the server.
...
work_mem|3932kB|Sets the maximum memory to be used for query workspaces.
